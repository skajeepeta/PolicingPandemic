...
```{r}
#Read in 2020 data
arrests1 <- read_csv("/Users/sk4538/Sync/Mass Incarceration and Public Health/COVID and arrests/Data/Arrests of interest/NYPD_Arrest_Data__Year_to_Date_July.csv")
arrests1$ARREST_DATE <- as.Date(arrests1$ARREST_DATE, "%m/%d/%Y")

#Read in historical data (pre-2020)
arrests2 <- read_csv("/Users/sk4538/Sync/Mass Incarceration and Public Health/COVID and arrests/Data/Arrests of interest/NYPD_Arrests_Data__Historic.csv")
...
```{r}
#Read in 2020 data
totarrests1 <- read_csv("/Users/sk4538/Sync/Mass Incarceration and Public Health/COVID and arrests/Data/All arrests/Total Arrests_Year_to_Date_July.csv")

#Read in historical data (pre-2020)
totarrests2 <- read_csv("/Users/sk4538/Sync/Mass Incarceration and Public Health/COVID and arrests/Data/All arrests/Total Arrests_Historic.csv")
...
dailys <- summons %>% group_by(ARREST_DATE, postCovid, PERP_RACE) %>% tally()
dailys$summons <- dailys$n
dailys$n <- NULL

#Join arrests and summonses
daily <- daily %>% left_join(dailys, by=c("ARREST_DATE","postCovid", "PERP_RACE"))
...
daily <- daily %>% group_by(ARREST_DATE, postCovid, black) %>% summarise(arrestcount = sum(n), summonscount=sum(summons))
```
...
daily$summonscount <- ifelse(is.na(daily$summonscount), 0, daily$summonscount)
...
zcta_sf <- read_sf("/Users/sk4538/Sync/Mass Incarceration and Public Health/COVID and arrests/Data/Shapefiles/ZCTAs/nyu_2451_34509.shp") %>%
...
zip <- read_csv("/Users/sk4538/Sync/Mass Incarceration and Public Health/COVID and arrests/Data/Neighborhood demographics/ZCTA/ZCTA_ACS_2018.csv")
...
zip2010 <- read_csv("/Users/sk4538/Sync/Mass Incarceration and Public Health/COVID and arrests/Data/Neighborhood demographics/ZCTA/zipLevelAsthma.csv")
...
zippop <- read_csv("/Users/sk4538/Sync/Mass Incarceration and Public Health/COVID and arrests/Data/Neighborhood demographics/ZCTA/Pop totals.csv")
...
#pull in borough
zipboro <- read_csv("/Users/sk4538/Sync/Mass Incarceration and Public Health/COVID and arrests/Data/Neighborhood demographics/ZCTA/zip_boro.csv")
...
```{r}
socialDistancing <- read_csv("/Users/sk4538/Sync/Mass Incarceration and Public Health/COVID and arrests/Data/Social distancing covariates/socialDistancing2.csv") #read social distancing data
...
```{r}
df311 <- read_csv("/Users/sk4538/Sync/Mass Incarceration and Public Health/COVID and arrests/Data/Social distancing covariates/df311.csv")
...
```{r}
summons <- read_csv("/Users/sk4538/Sync/Mass Incarceration and Public Health/COVID and arrests/Data/Summonses/NYPD_Criminal_Court_Summons_Incident_Level_Data__Year_To_Date_.csv")
...
```{r}
sqf <- read_csv("/Users/sk4538/Sync/Mass Incarceration and Public Health/COVID and arrests/Data/SQF/2011.csv")
summary(sqf)
...
##blackz
```{r}
m5 <- glm(formula = arrests ~ blackz + boro + offset(log(pop_estimate)), family=quasipoisson, data=arrests_SD)
summary(m5)
exp(m5$coefficients["blackz"])
exp(confint(m5))
```

##2010 sensitivity analysis
```{r}
m5 <- glm(formula = arrests ~ blackz_2010 + boro + offset(log(pop_estimate_2010)), family=quasipoisson, data=arrests_SD)
summary(m5)
exp(m5$coefficients["blackz_2010"])
exp(confint(m5))
```

##povertyz
```{r}
m7 <- glm(formula = arrests ~ povertyz + boro + offset(log(pop_estimate)), family=quasipoisson, data=arrests_SD)
summary(m7)
exp(m7$coefficients["povertyz"])
exp(confint(m7))
```

##2010 sensitivity analysis
```{r}
m7 <- glm(formula = arrests ~ povertyz_2010 + boro + offset(log(pop_estimate_2010)), family=quasipoisson, data=arrests_SD)
summary(m7)
exp(m7$coefficients["povertyz_2010"])
exp(confint(m7))
```

##weighted_median_non_home_dwell_timez
```{r}
m7 <- glm(formula = arrests ~ weighted_median_non_home_dwell_timez + boro + offset(log(pop_estimate)), family=quasipoisson, data=arrests_SD)
summary(m7)
exp(m7$coefficients["weighted_median_non_home_dwell_timez"])
exp(confint(m7))
```

##2010 sensitivity analysis
```{r}
m7 <- glm(formula = arrests ~ weighted_median_non_home_dwell_timez + boro + offset(log(pop_estimate_2010)), family=quasipoisson, data=arrests_SD)
summary(m7)
exp(m7$coefficients["weighted_median_non_home_dwell_timez"])
exp(confint(m7))
```
...
##311 complaints
```{r}
m7 <- glm(formula = arrests ~ complaintsSD_ratez + boro + offset(log(pop_estimate)), family=quasipoisson, data=arrests_SD)
summary(m7)
exp(m7$coefficients["complaintsSD_ratez"])
exp(confint(m7))
```

##2010 sensitivity analysis
```{r}
m7 <- glm(formula = arrests ~ complaintsSD_ratez_2010 + boro + offset(log(pop_estimate_2010)), family=quasipoisson, data=arrests_SD)
summary(m7)
exp(m7$coefficients["complaintsSD_ratez_2010"])
exp(confint(m7))
```

##2011 SQF arrests
```{r}
m7 <- glm(formula = arrests ~ sqfratez + boro + offset(log(pop_estimate)), family=quasipoisson, data=arrests_SD)
summary(m7)
exp(m7$coefficients["sqfratez"])
exp(confint(m7))
```

##2010 sensitivity analysis
```{r}
m7 <- glm(formula = arrests ~ sqfratez + boro + offset(log(pop_estimate_2010)), family=quasipoisson, data=arrests_SD)
summary(m7)
exp(m7$coefficients["sqfratez"])
exp(confint(m7))
```
...
##blackz
```{r}
m5 <- glm(formula = summons ~ blackz + boro + offset(log(pop_estimate)), family=quasipoisson, data=arrests_SD2)
summary(m5)
exp(m5$coefficients["blackz"])
exp(confint(m5))
```
...
##2010 sensitivity analysis
```{r}
m5 <- glm(formula = summons ~ blackz_2010 + boro + offset(log(pop_estimate_2010)), family=quasipoisson, data=arrests_SD2)
summary(m5)
exp(m5$coefficients["blackz_2010"])
exp(confint(m5))
```
...
##povertyz
```{r}
m7 <- glm(formula = summons ~ povertyz + boro + offset(log(pop_estimate)), family=quasipoisson, data=arrests_SD2)
summary(m7)
exp(m7$coefficients["povertyz"])
exp(confint(m7))
```
...
##2010 sensitivity analysis
```{r}
m7 <- glm(formula = summons ~ povertyz_2010 + boro + offset(log(pop_estimate_2010)), family=quasipoisson, data=arrests_SD2)
summary(m7)
exp(m7$coefficients["povertyz_2010"])
exp(confint(m7))
```
...
##weighted_median_non_home_dwell_timez
```{r}
m7 <- glm(formula = summons ~ weighted_median_non_home_dwell_timez + boro + offset(log(pop_estimate)), family=quasipoisson, data=arrests_SD2)
summary(m7)
exp(m7$coefficients["weighted_median_non_home_dwell_timez"])
exp(confint(m7))
```
...
##2010 sensitivity analysis
```{r}
m7 <- glm(formula = summons ~ weighted_median_non_home_dwell_timez + boro + offset(log(pop_estimate_2010)), family=quasipoisson, data=arrests_SD2)
summary(m7)
exp(m7$coefficients["weighted_median_non_home_dwell_timez"])
exp(confint(m7))
```
...
##311 complaints
```{r}
m7 <- glm(formula = summons ~ complaintsSD_ratez + boro + offset(log(pop_estimate)), family=quasipoisson, data=arrests_SD2)
summary(m7)
exp(m7$coefficients["complaintsSD_ratez"])
exp(confint(m7))
```
...
##2010 sensitivity analysis
```{r}
m7 <- glm(formula = summons ~ complaintsSD_ratez_2010 + boro + offset(log(pop_estimate_2010)), family=quasipoisson, data=arrests_SD2)
summary(m7)
exp(m7$coefficients["complaintsSD_ratez_2010"])
exp(confint(m7))
```
...
##2011 SQF arrests
```{r}
m7 <- glm(formula = summons ~ sqfratez + boro + offset(log(pop_estimate)), family=quasipoisson, data=arrests_SD2)
summary(m7)
exp(m7$coefficients["sqfratez"])
exp(confint(m7))
```
...
##2010 sensitivity analysis
```{r}
m7 <- glm(formula = summons ~ sqfratez + boro + offset(log(pop_estimate_2010)), family=quasipoisson, data=arrests_SD2)
summary(m7)
exp(m7$coefficients["sqfratez"])
exp(confint(m7))
```